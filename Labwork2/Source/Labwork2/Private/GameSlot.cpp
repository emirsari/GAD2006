...
void AGameSlot::SpawnUnitHere(TSubclassOf<AUnitBase>& UnitClass)
{
	...
	AUnitBase* NewUnit = Cast<AUnitBase>(GWorld->SpawnActor(UnitClass, &Location)); // '&' address of FVector (location)
...
{
	Super::BeginPlay();
	OnClicked.AddUniqueDynamic(this, &AGameSlot::OnGridClicked); // function address &AGameSlot::OnGridClicked
